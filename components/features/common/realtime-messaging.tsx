    ...
    const fetchProjectData = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          const acceptedBid = bidsData.success
            ? bidsData.data.find(
                (b: any) =>
          ...
          const freelancerId = acceptedBid
            ? typeof acceptedBid.freelancerId === "object"
              ? acceptedBid.freelancerId._id
              : acceptedBid.freelancerId
            : null;

          const freelancerUser =
            acceptedBid && typeof acceptedBid.freelancerId === "object"
              ? acceptedBid.freelancerId
              : null;
    ...
    const fetchMessages = async () => {
      ...
      try {
        const res = await fetch(
          `/api/v1/chat/history/${activeParticipant._id}`,
          {
            headers: { Authorization: `Bearer ${user.accessToken}` },
          }
        );
          ...
          <div className="w-full md:w-80 flex-shrink-0 border-r bg-muted/10 flex flex-col">
            <div className="p-4 border-b">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Search..."
                  className="pl-9 bg-background/50 border-none shadow-sm focus-visible:ring-purple-500/20"
                />
              </div>
            </div>
              ...
              {participants.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-40 text-center p-4">
                  ...
                  <p className="text-xs text-muted-foreground/60 mt-1">
                    Chat will start when you connect with a freelancer or
                    client.
                  </p>
                </div>
              ) : (
                participants.map((participant) => (
                  ...
                  >
                    <div
                      className={`relative w-10 h-10 rounded-full flex items-center justify-center font-semibold shadow-sm text-sm transition-transform group-hover:scale-105 ${
                        activeParticipant?._id === participant._id
              ...
              <>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Active Now - {activeParticipant.role}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {loading ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center text-center p-8">
                      ...
                      <p className="text-sm text-muted-foreground max-w-xs">
                        Say hello to {activeParticipant.name} to discuss the
                        project details.
                      </p>
                    </div>
                  ) : (
                    messages.map((message, index) => {
                      ...
                      const isSequence = index > 0 && prevSenderId === senderId;
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              <span
                                className={`text-[10px] absolute bottom-1 right-2 opacity-0 group-hover:opacity-100 transition-opacity ${
                                  isMe
                                    ? "text-purple-100"
                                    : "text-muted-foreground"
                                }`}
                ...
                <div className="p-4 bg-background/80 backdrop-blur-md border-t">
                  ...
                  <p className="text-[10px] text-muted-foreground text-center mt-3">
                    Press{" "}
                    <kbd className="font-sans px-1 rounded bg-muted">Enter</kbd>{" "}
                    to send
                  </p>
                </div>
              </>
                ...
                <div className="max-w-md text-center space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      Select a conversation from the sidebar to chat instantly
                      with your team. History stays saved so you can catch up
                      anytime.
                    </p>
                  </div>
                </div>
