      ...
      <div className="flex flex-col h-full space-y-6">
        <div className="flex justify-between items-center">
            <div>
                ...
                <p className="text-muted-foreground">Share and manage project assets</p>
            </div>
        ...
        <Card className="border-2 border-dashed border-muted-foreground/25 bg-muted/5 hover:bg-muted/10 transition-colors">
            <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mt-1 mb-6">
                    Drag and drop your files here, or click to browse. Supports PDF, DOCX, JPG, PNG (Max 10MB)
                </p>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {selectedFile && (
                        <div className="flex items-center gap-2 bg-background border px-3 py-1.5 rounded-md text-sm">
                            ...
                            <Button size="icon" variant="ghost" className="h-5 w-5" onClick={(e) => {
                                e.stopPropagation();
                                setSelectedFile(null);
                            }}>
                                <X className="w-3 h-3" />
                    ...
                    <Card key={file.id} className="group hover:shadow-md transition-all duration-200">
                        <CardContent className="p-4 flex items-start gap-3">
                            <div className={`p-3 rounded-lg ${
                                file.type === 'pdf' ? 'bg-red-50 text-red-600 dark:bg-red-900/10 dark:text-red-400' :
                                file.type === 'docx' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/10 dark:text-blue-400' :
                                'bg-gray-50 text-gray-600 dark:bg-gray-800 dark:text-gray-400'
                            }`}>
                                <File className="w-6 h-6" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h5 className="font-medium truncate" title={file.name}>{file.name}</h5>
