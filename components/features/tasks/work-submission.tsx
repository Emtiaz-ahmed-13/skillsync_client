...
export default function WorkSubmission({
  ...
  const completedFeaturesCount = sprints.reduce((sum, s) => 
    sum + s.features.filter(f => f.status === "completed").length, 0
  );
  const inProgressFeaturesCount = sprints.reduce((sum, s) => 
    sum + s.features.filter(f => f.status === "in-progress").length, 0
  );
  const pendingFeaturesCount = sprints.reduce((sum, s) => 
    sum + s.features.filter(f => f.status === "pending").length, 0
  );
  const overallProgress = totalFeaturesCount > 0 
      ...
      <Card className="lg:col-span-1 h-full overflow-hidden flex flex-col border-r shadow-none rounded-none lg:rounded-l-xl lg:border-y lg:border-l">
        <CardHeader className="bg-muted/30 pb-4 border-b">
          <CardTitle className="text-lg flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><path d="M12 13v6"/><path d="M9 16l3 3 3-3"/></svg>
            Sprints
          </CardTitle>
        ...
        <CardContent className="p-0 flex-1 overflow-y-auto">
          <div className="flex flex-col">
            {sprints.map((s) => (
              ...
              >
                <div>
                  <h4 className={`font-semibold text-sm ${selectedSprint === s._id ? "text-primary" : ""}`}>
                    {s.title}
                  </h4>
                ...
                <Badge
                  variant={
                    ...
                    s.status === "in-progress" ? "secondary" : "outline"
                  }
        ...
        {selectedSprintData ? (
            <>
                ...
                <Card>
                    <CardHeader className="pb-4">
                        <div className="flex justify-between items-start">
                            <div>
                                <CardTitle className="text-2xl">{selectedSprintData.title}</CardTitle>
                                <CardDescription className="mt-1">{selectedSprintData.description}</CardDescription>
                            </div>
                             <div className="text-right">
                                <div className="text-sm font-medium text-muted-foreground">Sprint Status</div>
                                <Badge className={`mt-1 text-sm ${
                                     selectedSprintData.status === 'completed' ? 'bg-green-500 hover:bg-green-600' : 
                                     selectedSprintData.status === 'in-progress' ? 'bg-blue-500 hover:bg-blue-600' : ''
                                }`}>
                                    {selectedSprintData.status.toUpperCase().replace('-', ' ')}
                                </Badge>
                             </div>
                        </div>
                    </CardHeader>
                    <CardContent className="border-t pt-6">
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground mb-2">Features Completion</p>
                                <div className="flex items-end gap-2">
                                    <span className="text-3xl font-bold text-primary">
                                        {completedFeatures.length}
                                        <span className="text-lg text-muted-foreground font-normal">/{selectedSprintData.features.length}</span>
                                    </span>
                                </div>
                                <Progress value={selectedSprintData.features.length > 0 ? (completedFeatures.length / selectedSprintData.features.length) * 100 : 0} className="h-2 mt-2" />
                            </div>
                            
                            <div className="flex flex-col justify-center">
                                <div className="flex items-center justify-between text-sm mb-2">
                                    <span className="text-muted-foreground">Start Date</span>
                                    <span className="font-mono">{new Date(selectedSprintData.startDate).toLocaleDateString()}</span>
                                </div>
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-muted-foreground">End Date</span>
                                    <span className="font-mono">{new Date(selectedSprintData.endDate).toLocaleDateString()}</span>
                                </div>
                            </div>
                             ...
                             <div className="flex items-center justify-center bg-muted/30 rounded-lg p-2">
                                <div className="text-center">
                                    ...
                                    <p className="text-xl font-bold">{remainingFeatures.length} Features</p>
                                </div>
                            </div>
                         </div>
                    </CardContent>
                ...
                {workSubmissions.some(s => {
                     const sId = typeof s.sprintId === 'object' && s.sprintId !== null ? (s.sprintId as any)._id : s.sprintId;
                     return sId === selectedSprint;
                }) ? (
                    /* ALREADY SUBMITTED VIEW */
                     <div className="space-y-4">
                        <h3 className="text-lg font-semibold flex items-center gap-2">
                             <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                            Submission History
                        </h3>
                        {workSubmissions
                            .filter((sub) => {
                            const sprintId = typeof sub.sprintId === 'object' && sub.sprintId !== null 
                                ? (sub.sprintId as any)._id 
                                : sub.sprintId;
                            return sprintId === selectedSprint;
                            })
                            .map((submission) => (
                            <Card key={submission._id} className="border-green-200 bg-green-50/10 dark:border-green-900/50">
                            <CardHeader>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Status: <span className="font-semibold text-foreground">{submission.status.toUpperCase()}</span>
                                    </p>
                                </div>
                                {submission.status === 'pending' && (
                                    <Badge variant="outline" className="border-yellow-500 text-yellow-600 bg-yellow-50">Pending Review</Badge>
                                )}
                            ...
                            <CardContent>
                                <div className="grid md:grid-cols-2 gap-4">
                                     <a href={submission.githubLink} target="_blank" className="flex items-center gap-2 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                                     ...
                                     {submission.liveLink && (
                                        <a href={submission.liveLink} target="_blank" className="flex items-center gap-2 p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                         <Card className="border-primary/20 shadow-md">
                            ...
                            <CardTitle className="text-xl flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 12v9"/><path d="m16 16-4-4-4 4"/></svg>
                                Submit Work for {selectedSprintData.title}
                            </CardTitle>
                            <CardDescription>
                                Ensure your code is pushed to GitHub and deployed before submitting.
                            </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6 pt-6">
                                ...
                                <div className="space-y-3">
                                    ...
                                    {remainingFeatures.length > 0 && (
                                         <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-lg text-xs">
                                            ...
                                            Warning: {remainingFeatures.length} features are not marked as completed. You can still submit, but approval may be pending via discussion.
                                         </div>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="absolute left-3 top-2.5 w-4 h-4 text-muted-foreground" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
                                        <Input
                                            className="pl-9"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                         <svg xmlns="http://www.w3.org/2000/svg" className="absolute left-3 top-2.5 w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                        <Input
                                            className="pl-9"
