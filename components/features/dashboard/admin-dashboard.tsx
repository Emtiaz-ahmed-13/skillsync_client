...
export default function AdminDashboardClient() {
  ...
  const handleApproveProject = async (
    ...
    try {
      ...
      toast.error(`Error ${status}ing project: ${err}`);
    }
  };
  ...
  const handleApproveArticle = async (articleId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "Failed to approve article");
      }
    } catch (err) {
  ...
  const handleRejectArticle = async (articleId: string, reason?: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "Failed to reject article");
      }
    } catch (err) {
        ...
        <div className="space-y-8 py-4">
          ...
          >
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle>Pending Articles for Approval</CardTitle>
                  <CardDescription>
                    {pendingArticles.length} article(s) waiting for review
                  </CardDescription>
                </div>
                <Badge variant="outline">
